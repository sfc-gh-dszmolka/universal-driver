...
fn test_database_set_option_string() {
    ...
    let db = client.database_new().unwrap();
    client.database_set_option_string(db.clone(), "test_option".to_string(), "test_value".to_string()).unwrap();
    client.database_release(db).unwrap();
}
...
fn test_database_set_option_bytes() {
    ...
    client.database_set_option_bytes(db.clone(), "test_option".to_string(), test_bytes).unwrap();
    client.database_release(db).unwrap();
}
...
fn test_database_set_option_int() {
    ...
    let db = client.database_new().unwrap();
    client.database_set_option_int(db.clone(), "test_option".to_string(), 42).unwrap();
    client.database_release(db).unwrap();
}
...
fn test_database_set_option_double() {
    ...
    let db = client.database_new().unwrap();
    client.database_set_option_double(db.clone(), "test_option".to_string(), 3.14.into()).unwrap();
    client.database_release(db).unwrap();
}
...
fn test_database_lifecycle() {
    ...
    // Set various options
    client.database_set_option_string(db.clone(), "driver".to_string(), "test_driver".to_string()).unwrap();
    client.database_set_option_int(db.clone(), "timeout".to_string(), 30).unwrap();
...
fn test_connection_set_option_string() {
    ...
    let conn = client.connection_new().unwrap();
    client.connection_set_option_string(conn.clone(), "username".to_string(), "test_user".to_string()).unwrap();
    client.connection_release(conn).unwrap();
}
...
fn test_connection_set_option_bytes() {
    ...
    client.connection_set_option_bytes(conn.clone(), "cert".to_string(), test_bytes).unwrap();
    client.connection_release(conn).unwrap();
}
...
fn test_connection_set_option_int() {
    ...
    let conn = client.connection_new().unwrap();
    client.connection_set_option_int(conn.clone(), "port".to_string(), 5432).unwrap();
    client.connection_release(conn).unwrap();
}
...
fn test_connection_set_option_double() {
    ...
    let conn = client.connection_new().unwrap();
    client.connection_set_option_double(conn.clone(), "timeout_seconds".to_string(), 30.5.into()).unwrap();
    client.connection_release(conn).unwrap();
}
...
fn test_connection_init() {
    ...
    let conn = client.connection_new().unwrap();
    client.connection_init(conn.clone(), "test_db".to_string()).unwrap();
    client.connection_release(conn).unwrap();
}
...
fn test_connection_get_info() {
    ...
    let _info_result = client.connection_get_info(conn.clone(), info_codes).unwrap();
...
fn test_connection_get_objects() {
    ...
    let _objects = client.connection_get_objects(
        conn.clone(),
        1, // depth
        "catalog".to_string(),
        "schema".to_string(),
...
fn test_connection_get_table_schema() {
    ...
    let _schema = client.connection_get_table_schema(
        conn.clone(),
        "catalog".to_string(),
        "schema".to_string(),
        "table".to_string()
...
fn test_connection_lifecycle() {
    ...
    client.connection_set_option_string(conn.clone(), "host".to_string(), "localhost".to_string()).unwrap();
    client.connection_set_option_int(conn.clone(), "port".to_string(), 5432).unwrap();
    client.connection_set_option_string(conn.clone(), "username".to_string(), "test_user".to_string()).unwrap();
    ...
    // Get driver info
    let info_codes = vec![InfoCode::DRIVER_NAME, InfoCode::DRIVER_VERSION];
    let _info = client.connection_get_info(conn.clone(), info_codes).unwrap();
...
fn test_statement_set_sql_query() {
    ...
    let stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_sql_query(stmt.clone(), "SELECT 1".to_string()).unwrap();
...
fn test_statement_set_substrait_plan() {
    ...
    client.statement_set_substrait_plan(stmt.clone(), plan_bytes).unwrap();
...
fn test_statement_prepare() {
    ...
    let stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_sql_query(stmt.clone(), "SELECT ? as value".to_string()).unwrap();
...
fn test_statement_set_option_string() {
    ...
    let stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_option_string(stmt.clone(), "query_timeout".to_string(), "30".to_string()).unwrap();
...
fn test_statement_set_option_bytes() {
    ...
    client.statement_set_option_bytes(stmt.clone(), "binary_option".to_string(), option_bytes).unwrap();
...
fn test_statement_set_option_int() {
    ...
    let stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_option_int(stmt.clone(), "max_rows".to_string(), 1000).unwrap();
...
fn test_statement_set_option_double() {
    ...
    let stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_option_double(stmt.clone(), "timeout_seconds".to_string(), 30.5.into()).unwrap();
...
fn test_statement_bind() {
    ...
    client.statement_bind(stmt.clone(), record_batch_bytes).unwrap();
...
fn test_statement_bind_stream() {
    ...
    let stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_sql_query(stmt.clone(), "INSERT INTO table VALUES (?)".to_string()).unwrap();
    
    // Mock Arrow stream in IPC format
    let stream_bytes = vec![0x41, 0x52, 0x52, 0x4F, 0x57, 0x31]; // Mock stream
    client.statement_bind_stream(stmt.clone(), stream_bytes).unwrap();
...
fn test_statement_execute_query() {
    ...
    let stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_sql_query(stmt.clone(), "SELECT 1 as value".to_string()).unwrap();
...
fn test_statement_execute_partitions() {
    ...
    let stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_sql_query(stmt.clone(), "SELECT * FROM large_table".to_string()).unwrap();
...
fn test_statement_read_partition() {
    ...
    if !partitions.partitions.is_empty() {
        ...
        let _stream_ptr = client.statement_read_partition(stmt.clone(), partition_descriptor).unwrap();
    }
...
fn test_statement_lifecycle() {
    ...
    client.statement_set_option_int(stmt.clone(), "max_rows".to_string(), 100).unwrap();
    client.statement_set_option_string(stmt.clone(), "query_timeout".to_string(), "30".to_string()).unwrap();
    
    // Set and prepare query
    client.statement_set_sql_query(stmt.clone(), "SELECT ? as value, ? as name".to_string()).unwrap();
    client.statement_prepare(stmt.clone()).unwrap();
...
fn test_full_adbc_workflow() {
    ...
    // Get driver info
    let info_codes = vec![InfoCode::DRIVER_NAME, InfoCode::DRIVER_VERSION, InfoCode::VENDOR_NAME];
    let _info = client.connection_get_info(conn.clone(), info_codes).unwrap();
    
    // Statement lifecycle for DDL
    let ddl_stmt = client.statement_new(conn.clone()).unwrap();
    client.statement_set_sql_query(ddl_stmt.clone(), "CREATE TABLE test (id INT, name TEXT)".to_string()).unwrap();
    ...
    client.statement_set_sql_query(insert_stmt.clone(), "INSERT INTO test VALUES (?, ?)".to_string()).unwrap();
    client.statement_prepare(insert_stmt.clone()).unwrap();
    
    let record_batch = vec![0x41, 0x52, 0x52, 0x4F, 0x57]; // Mock Arrow data
    client.statement_bind(insert_stmt.clone(), record_batch).unwrap();
    ...
    client.statement_set_sql_query(select_stmt.clone(), "SELECT * FROM test".to_string()).unwrap();
